...
// Dot matrix row (anode) connections are connected to Port0 pins P0.0 to P0.7
#define ROWS 0
...
{
    WRITEBYTE(IODIR0, ROWS, 0xFF);   // Set lower 8 bits of Port0 as outputs
}
...
void display_char(u8 ch, u32 delay, u8 dmno)
{
    ...
    // Repeat display for the specified refresh duration
    for(dly = 0; dly < delay; dly++)
    {
        ...
        for(i = 0; i < 8; i++)
        {
            ...
            // Send corresponding row data to the selected display
            if(dmno == DM1)
            ...
            delay_us(40);                        // Small delay for persistence
        }
    }
}
...
void display_str(u8 *str, u32 delay)
{
    ...
    for(dly = 0; dly < delay; dly++)
    {
        // Scan each of the 8 rows
        for(i = 0; i < 8; i++)
        {
            WRITEBYTE(IOPIN0, ROWS, 0x00);   // Turn OFF all rows

            // Send one character to each dot matrix display
            ...
            delay_ms(1);                         // Hold row for visibility
        }
    }
}