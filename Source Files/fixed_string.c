...
// Initialize the dot matrix row pins as output
void Init_dotmatrix()
{
    // Configure the row pins as output pins
    WRITEBYTE(IODIR0, ROWS, 0xFF);
}


// Display a 4-character string on the dot matrix
void display_str(u8 *str, u32 delay)
{
    u8 i;
    u32 dly;

    // Refresh the display for the specified duration
    for(dly = 0; dly < delay; dly++)
    {
        ...
        for(i = 0; i < 8; i++)
        {
            ...
            // Send row data of each character to the four dot matrix displays
